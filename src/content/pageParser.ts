  ...
  if (openingTagsLength === closingTagsLength) {
    ...
    if (openingTags && closingTags) {
      ...
      if (lastOpeningTag > lastClosingTag) {
        ...
        // Update lastIndex to current index
        lastIndex = lastOpeningTag + openingTags[openingTagsLength - 1].length;
      }
    ...
    if (openingTagsLength > closingTagsLength) {
      ...
      for (let i = 0; i < difference; i += 1) {
        ...
        newString = paragraph.substring(endIndex + 1, lastIndex) + newString;
        ...
        // Add actual tag to newString
        newString = paragraph.substring(beginIndex, endIndex + 1) + newString;
...
function removeEmptyTags(str: string): string {
  ...
  const emptyTagRegex = /<wm[^<]*id="watermark-(\d+)-(\d+)"[^>]*(?<!\/)>[\s\n\t\t]*<\/wm>/;
        ...
        if (el.id !== `watermark-${wmIndex}-0`) return el;
          ...
          return {
            ...
            subTags: el.subTags?.filter(
              subEl => subEl.id !== `watermark-${wmIndex}-${subParIndex}`
            ),
          };
        }
      });
...
function populateParagraphList() {
  ...
  const markTagsWithId = Array.from(markTags).filter(el => el.id !== undefined);
...
function showWatermarking() {
  wmParagraphs.forEach(el => {
    if (el.watermark) {
      if (el.watermark.verificationStatus !== VerificationStatus.UNKNOWN) {
        const className =
