...
function applyExportPDF(document) {
  ...
  const tmpFile = docFile.makeCopy("watermarked_"+fileName); //TODO: Genero nuovo invece che fare copia potrebbe sistemare formattazione
  const tmpDoc = DocumentApp.openById(tmpFile.getId()); //TODO: Nome file output contenga document ID O testo custom da input
  ...
  const watermarkedBody = encodeText(originalText, fakePayload);
